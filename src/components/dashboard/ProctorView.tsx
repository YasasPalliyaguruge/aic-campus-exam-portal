...
export const ProctorView = () => {
  ...
  const buildProofHtml = (rows: string[][], imageSrc: string, isEmbedded: boolean) => {
...
<head>
  <meta charset="utf-8" />
  <title>AIC Campus Screen Capture Evidence</title>
  <style>
    body { margin: 0; background: #f8fafc; color: #111827; font-family: Arial, sans-serif; }
    main { max-width: 1200px; margin: 0 auto; padding: 32px; }
    h1 { margin: 0; font-size: 32px; }
    h2 { margin: 6px 0 24px; color: #4b5563; font-size: 20px; }
    table { width: 100%; border-collapse: collapse; margin: 24px 0; background: #fff; border: 1px solid #e5e7eb; }
    th, td { padding: 12px 14px; border-bottom: 1px solid #e5e7eb; text-align: left; vertical-align: top; }
    th { width: 210px; color: #64748b; font-size: 13px; text-transform: uppercase; letter-spacing: .04em; }
    img { display: block; width: 100%; height: auto; background: #000; border: 1px solid #cbd5e1; }
    footer { margin-top: 18px; color: #64748b; font-size: 13px; line-height: 1.5; }
    @media print { body { background: #fff; } main { padding: 0; } }
  </style>
</head>
<body>
  <main>
    <h1>AIC Campus Exam Portal</h1>
    <h2>Screen Capture Evidence</h2>
    <table>${metadataRows}</table>
    <img src="${imageSrc}" alt="Latest student screen capture" />
    <footer>
      This proof file was generated by the proctor browser from the latest captured screenshot.
      ${isEmbedded
        ? 'The screenshot is embedded in this file; no extra proof document is stored in Firestore.'
        : 'Fallback mode: the screenshot is referenced from the existing secure Storage URL because embedded proof generation was unavailable.'}
    </footer>
  </main>
</body>
</html>`;
  ...
  const handleDownloadScreenCaptureProof = async (
    ...
    try {
      ...
      const imageDataUrl = `data:${proof.contentType || 'image/jpeg'};base64,${proof.imageBase64}`;
      const html = buildProofHtml(rows, imageDataUrl, true);
      const proofBlob = new Blob([html], { type: 'text/html;charset=utf-8' });
      downloadBlob(
        proofBlob,
        `${sanitizeFileName(student?.name || session.studentId)}_${sanitizeFileName(exam?.title || session.examId)}_${capturedPart}_screen-proof.html`,
      );
    } catch (error: any) {
      if (session.screenCapture?.imageUrl) {
        ...
        const rows = [
          ...
          ['Fallback reason', error?.message || 'Embedded proof generation unavailable'],
        ];
        const html = buildProofHtml(rows, withCacheBust(session.screenCapture.imageUrl, session.screenCapture.capturedAt), false);
        downloadBlob(
